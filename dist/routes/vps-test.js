...
router.post('/provision-worker', async (req, res) => {
    const logs = [];
    const log = (msg) => {
        console.log(msg);
        logs.push(msg);
    ...
    try {
        ...
        provisioner.provisionVPS({
            ...
            supabase_1.db.updateWorker(workerId, { status: 'error' }).catch(() => { });
        });
...
router.get('/hetzner-health', async (req, res) => {
    try {
        const hetznerToken = process.env.HETZNER_API_TOKEN;
        if (!hetznerToken) {
        ...
        const hetzner = new hetznerService_1.HetznerService(hetznerToken);
...
router.post('/create-test-server', async (req, res) => {
    const logs = [];
    const log = (msg) => {
        console.log(msg);
        logs.push(msg);
...
router.post('/test-provision', auth_1.authenticate, async (req, res) => {
    const logs = [];
    const addLog = (msg) => {
        console.log(msg);
    ...
    try {
        ...
        let hetzner;
        try {
            hetzner = new hetznerService_1.HetznerService(hetznerToken);
...
//# sourceMappingURL=vps-test.js.map