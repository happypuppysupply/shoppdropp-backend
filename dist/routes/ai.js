...
router.post('/configure', auth_1.authenticate, (0, express_validator_1.body)('provider').isIn(VALID_PROVIDERS).withMessage('Invalid provider'), (0, express_validator_1.body)('model').notEmpty().withMessage('Model is required'), (0, express_validator_1.body)('apiKey').notEmpty().withMessage('API key is required'), async (req, res) => {
    try {
        const user = req.user;
        ...
        await supabase_1.db.saveAIConfig(user.id, {
            provider,
            model,
...
router.get('/config', auth_1.authenticate, async (req, res) => {
    try {
        const user = req.user;
        const config = await supabase_1.db.getAIConfig(user.id);
        if (!config) {
...
function getValidModels(provider) {
    const models = {
