...
exports.VPSProvisioner = void 0;
exports.createVPSProvisioner = createVPSProvisioner;
...
class VPSProvisioner {
    hetzner;
    sshPrivateKey;
    constructor(hetznerService, sshPrivateKey, sshPublicKey) {
    ...
    async provisionVPS(config) {
        ...
        try {
            ...
            try {
                ...
                const similarServer = existingServers.find(s => s.name.startsWith(`shoppdropp-worker-${config.workerId.slice(0, 8)}`));
                if (similarServer) {
    ...
    async logStep(workerId, stepNumber, stepName, progress, message) {
     ...
     */
    async getWorkerLogs(workerId, limit = 100) {
     ...
     */
    async getLatestWorkerLog(workerId) {
        const { data, error } = await supabase_1.supabase
            .from('worker_logs')
            .select('*')
            .eq('worker_id', workerId)
    ...
    async installOpenClaw(ipAddress, config) {
...
while true; do
    ...
    async ensureSSHKey() {
        try {
            ...
            const existingKey = keys.find((k) => k.name === 'shoppdropp-render') ||
                keys.find((k) => k.name === 'shoppdropp-render-fixed') ||
                keys.find((k) => k.name === 'shoppdropp-render-rsa') ||
                keys.find((k) => k.name === 'render');
            if (existingKey) {
            ...
            if (publicKeyBody) {
                const matchedByFingerprint = keys.find((k) => k.public_key && k.public_key.includes(publicKeyBody.substring(0, 50)));
...
`;
    ...
    async destroyVPS(serverId, workerId) {
        try {
...
exports.VPSProvisioner = VPSProvisioner;
// Factory function
function createVPSProvisioner() {
    ...
    const hetznerService = new hetznerService_1.HetznerService(hetznerToken);
    return new VPSProvisioner(hetznerService, keys.privateKey, keys.publicKey);
}
//# sourceMappingURL=vpsProvisioner.js.map